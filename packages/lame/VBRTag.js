...
export function VBRTag() {

    let lame;
    let bs;
    let v;

    this.setModules = (_lame, _bs, _v) => {
        lame = _lame;
        bs = _bs;
        v = _v;
    };
    ...
    const LAMEHEADERSIZE = (VBRHEADERSIZE + 9 + 1 + 1 + 8
    + 1 + 1 + 3 + 1 + 1 + 2 + 4 + 2 + 2);
     ...
     * x^16+x^15+x^2+1
     */
    const crc16Lookup = [0x0000, 0xC0C1, 0xC181, 0x0140,
    ...
    this.InitVbrTag = gfp => {
        ...
        const totalFrameSize = ((gfp.version + 1) * 72000 * kbps_header)
            / gfp.out_samplerate;
    ...
    this.updateMusicCRC = (crc, buffer, bufferPos, size) => {
    ...
    function putLameVBR(gfp, musicLength, streamBuffer, streamBufferPos, crc) {
        ...
        const lowpass = 0 | (((gfp.lowpassfreq / 100.0) + .5) > 255 ? 255
        ...
        if (gfc.findPeakSample)
            peakSignalAmplitude = Math
                .abs(0 | ((( gfc.PeakSample) / 32767.0) * (2 ** 23) + .5));
    ...
    this.getLameTagFrame = (gfp, buffer) => {
        const gfc = gfp.internal_flags;
        ...
        const streamSize = (gfc.VBR_seek_table.nBytesWritten + gfc.VBR_seek_table.TotalFrameSize);
