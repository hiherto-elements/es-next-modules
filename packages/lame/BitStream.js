...
BitStream.EQ = (a, b) => (Math.abs(a) > Math.abs(b)) ? (Math.abs((a) - (b)) <= (Math
    .abs(a) * 1e-6))
    : (Math.abs((a) - (b)) <= (Math.abs(b) * 1e-6));

BitStream.NEQ = (a, b) => !BitStream.EQ(a, b);
...
export function BitStream() {
    ...
    this.setModules = (_ga, _mpg, _ver, _vbr) => {
        ga = _ga;
        mpg = _mpg;
        ver = _ver;
        vbr = _vbr;
    ...
    this.CRC_writeheader = (gfc, header) => {
        let crc = 0xffff;
     ...
     * </PRE>
     */
    this.copy_buffer = (gfc, buffer, bufferPos, size, mp3data) => {
