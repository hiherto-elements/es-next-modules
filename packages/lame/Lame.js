...
export function Lame() {
    ...
    this.nearestBitrateFullIndex = bitrate => {
        /* borrowed from DM abr presets */

        const full_bitrate_table = [8, 16, 24, 32, 40, 48, 56, 64, 80,
    ...
    function optimum_bandwidth(lh, bitrate) {
         ...
         * </PRE>
         */
        const freq_map = [new BandPass(8, 2000),
    ...
    this.lame_init_params = gfp => {
        ...
        switch (gfp.VBR) {
            ...
            case VbrMode.vbr_mtrh:
            {
                ...
                const cmp = [5.7, 6.5, 7.3, 8.2, 10, 11.9, 13, 14,
                    15, 16.5];
        ...
        const j = gfc.samplerate_index + (3 * gfp.version) + 6
        ...
        gfc.ATH.aaSensitivityP = 10.0 ** (gfp.athaa_sensitivity / -10.0);
    ...
    this.lame_encode_buffer = (gfp, buffer_l, buffer_r, nsamples, mp3buf, mp3bufPos, mp3buf_size) => {
        const gfc = gfp.internal_flags;
    ...
    function lame_encode_frame(gfp, inbuf_l, inbuf_r, mp3buf, mp3bufPos, mp3buf_size) {
        const ret = self.enc.lame_encode_mp3_frame(gfp, inbuf_l, inbuf_r, mp3buf,
    ...
    function fill_buffer_resample(gfp, outbuf, outbufPos, desired_len, inbuf, in_bufferPos, len, num_used, ch) {
        ...
        let bpc = gfp.out_samplerate
            / gcd(gfp.out_samplerate, gfp.in_samplerate);
        if (bpc > LameInternalFlags.BPC)
            bpc = LameInternalFlags.BPC;

        const intratio = (Math.abs(gfc.resample_ratio
